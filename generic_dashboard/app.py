...
st.write(
    """
    CSV·Excel·JSON 파일을 올리면 **품질 점검 → 정제 → 시각화 → 그룹 집계 → 통계·시계열 분석 → 보고서**까지
    한 곳에서 진행할 수 있는 데이터 분석 대시보드입니다.
    수업에서 다룬 기본 분석 흐름을 중심으로 피드백받기 좋게 정리했습니다.
    """
)
...
cards = [
    ...
    ("📋", "품질 점검", "결측·중복·타입 확인과 분석 후보 변수를 점검합니다.", "데이터 품질 점검"),
    ...
    ("📈", "통계 분석", "t검정·ANOVA·상관분석을 수행합니다.", "통계 분석"),
    ("⏱️", "시계열 분석", "날짜 컬럼을 기준으로 기간별 흐름을 확인합니다.", "시계열 분석"),
